...
## おでんとスプラの後に始まった深夜セッション

2月14日、土曜日。バレンタイン。

おでんを仕込んで、スプラトゥーンを数戦やって、いい感じに脳がほぐれた夜11時過ぎ。ふと、ずっと気になっていたプロジェクトを開いた。

**cc-memory** ── 自分が作ったMCPメモリサーバーだ。

AIエージェントが「記憶」を持てるようにするツール。アイデア自体は悪くなかった。問題は、**33,000行**という圧倒的な過剰設計だった。3層メモリアーキテクチャ、OODAループ、DIKWモデル、攻殻機動隊のタチコマに着想を得た同期機構……。コンセプトは壮大だったが、正直なところ、自分でも使いこなせていなかった。

そこで、AIエージェントのFalakに聞いてみた。
...
バレンタインの夜、AIに心をえぐられるとは思わなかった。でも、この容赦ないレビューが、すべての始まりだった。

## 「エージェンティックAI」という言葉の罠

最近、どこを見ても「エージェンティックAI」という言葉が飛び交っている。LLMにツールを持たせて自律的にタスクを実行させる──言葉にすると簡単だが、実現は想像以上に難しい。

僕はインフラエンジニアとして25年やってきた。その経験を活かして社内で「Agentic AI Operations」という分科会を立ち上げ、エージェンティックAIの可能性を探ってきた。でも正直、具体的な成果物がなかった。議論は盛り上がるのに、動くものが出てこない。
...
## インフラ組織 = マルチエージェントの設計図

Falakのレビューを受けて悶々としていたとき、ふと気づいた。

**インフラの組織設計って、そのままマルチエージェントの設計図じゃないか？**

25年間、インフラ組織を作ってきた。Webチーム、DBチーム、メール基盤チーム、インフラチーム。それぞれの専門家がいて、マネージャーがタスクを分解して割り当てる。

これをそのままエージェントに置き換えると：
...
インフラ組織だって同じだ。Confluenceの共有ページはみんな見れるけど、個人の作業メモは本人しか見ない。他チームの作業メモを勝手に覗くのはマナー違反だし、そもそもアクセス権がない。

25年間、人間の組織で当たり前にやってきたことを、AIエージェントでもやればいい。
...
深夜3時、33,000行が989行になった。**97%の削減**。そして、v1より明らかに使いやすい。

## Dogfooding: cc-memoryでcc-memoryを開発する

ここからが本番だ。

作ったツールの実証として、**cc-memory自身の開発にcc-memoryを使う**ことにした。いわゆるdogfooding。自分のドッグフードを自分で食べる。
...
インフラ組織で言えば、DBチームの作業メモをWebチームが勝手に見ようとして、アクセス権で弾かれるのと同じ。当たり前の動作だけど、これがマルチエージェント環境で正しく動くのを見ると、設計が正しかったことを確信できる。

worker-storageが`storage.ts`を書き、worker-apiが`tools.ts`を書く。2体のエージェントが並行で、衝突なく、同じプロジェクトのコードを書いている。

**バレンタインの夜に、AIエージェント2体が並行でコードを書いていた。**
...
33,000行より989行の方が実際に動く。これは冗談じゃなくて、本当にそう。
...
25年間インフラ組織を設計・運営してきた経験が、そのままマルチエージェントの設計図になる。マネージャーとワーカーの関係、共有知識と個人知識の分離、アクセス権限の設計──全部、人間の組織でやってきたことだ。
...
机上の空論ではなく、実際に動いた。2体のエージェントが並行でコードを書き、共有メモリで要件を共有し、個人メモリで作業ログを記録し、権限モデルで適切にアクセス制御された。
...
25年のインフラ経験を、AIエージェントの世界に注ぎ込む。まだまだ面白くなりそうだ。

## まとめ

バレンタインの夜、おでんとスプラトゥーンの後に始まった深夜セッション。

AIエージェントに容赦ないレビューをもらい、33,000行を989行に書き直し、マルチエージェント開発のdogfoodingまで完了した。3時間で。

「エージェンティックAI」は流行り言葉じゃない。脳と手足と記憶とトリガーのループを、ちゃんと設計して実装すれば、本当に動く。そして、その設計図は意外なところにある──25年間やってきた、インフラ組織の運営そのものだ。
